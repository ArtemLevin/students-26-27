...
const dashboardPaths=indexes.filter(file=>file.endsWith('/site/index.html')&&!file.includes('/chemistry/')&&!file.includes('/nikol_sarkisyants/')&&!file.includes('/nastya_pavlova/')&&!file.includes('/ekaterina_gnedkova/'));
...
console.log(`✓ index inventory: ${indexes.length} entry pages covered, ${dashboardPaths.length} shared dashboards migrated, 1 bespoke EGE dashboard, ${adapterPaths.length} practice adapters`);
