...
export const LESSONS=Object.freeze([
  ...
  },
  {
    ...
    materials:{
      ...
      review:'../review_docs/review_23.08.26.pdf'
    }
  },
  {
    date:'2026-08-18',
...
export function sortedLessons(lessons=LESSONS){
  return [...lessons].sort(compareLessonsNewestFirst);
}

export function getLatestLesson(lessons=LESSONS){
  return sortedLessons(lessons)[0]||null;
}

export function getRecentLessons(lessons=LESSONS,limit=RECENT_LIMIT){
  return sortedLessons(lessons).slice(0,Math.max(0,limit));
}

export function getArchiveLessons(lessons=LESSONS,limit=RECENT_LIMIT){
  return sortedLessons(lessons).slice(Math.max(0,limit));
}

export function paginateArchive(lessons=LESSONS,pageIndex=0,pageSize=ARCHIVE_PAGE_SIZE){
  const archive=getArchiveLessons(lessons);
  const safeSize=Math.max(1,Number(pageSize)||ARCHIVE_PAGE_SIZE);
  const pageCount=Math.max(1,Math.ceil(archive.length/safeSize));
  const safeIndex=Math.max(0,Math.min(pageCount-1,Number(pageIndex)||0));
  const start=safeIndex*safeSize;
  return {
    items:archive.slice(start,start+safeSize),
    pageIndex:safeIndex,
    pageCount,
...
function parseIsoDate(isoDate){
  ...
  if(!match)throw new Error(`Invalid lesson date: ${isoDate}`);
  const year=Number(match[1]);
  const month=Number(match[2]);
  const day=Number(match[3]);
  if(month<1||month>12||day<1||day>31)throw new Error(`Invalid lesson date: ${isoDate}`);
...
export function formatLongDateRu(isoDate){
  const {year,month,day}=parseIsoDate(isoDate);
  return `${day} ${MONTHS_GENITIVE[month-1]} ${year}`;
}

export function validateLessonRegistry(lessons=LESSONS){
  if(!Array.isArray(lessons)||lessons.length===0)throw new Error('Lesson registry is empty');
  const dates=new Set();
  const hrefs=new Set();
  let previousDate=null;

  lessons.forEach((lesson,index)=>{
    parseIsoDate(lesson.date);
    if(!lesson.href||!lesson.title||!lesson.navTitle)throw new Error(`Lesson ${index} is incomplete`);
    ...
    if(previousDate!==null&&lesson.date>previousDate)throw new Error('Lesson registry must be sorted newest-first');
  ...
  if(!latest.summary||!Array.isArray(latest.topics)||latest.topics.length===0)throw new Error('Latest lesson requires summary and topics');
  if(!Array.isArray(latest.outcomes)||latest.outcomes.length===0)throw new Error('Latest lesson requires outcomes');
  if(!latest.materials||!latest.materials.pdf||!latest.materials.tex)throw new Error('Latest lesson requires PDF and TeX materials');
