...
Главное правило:
\[
\boxed{\text{координаты конца} - \text{координаты начала}}
\]
по каждой координате.

\subsection*{\color{darkaccent}Геометрический смысл}

Координаты вектора показывают его смещение:
...
{Для \(\vct c=(x;y)\) вычислите
\(\modv{\vct c}=\sqrt{x^2+y^2}\)};

\node[flowbranch,below=16mm of target,xshift=40mm] (coords)
{Запишите найденные координаты вектора как окончательный результат};
...
Знак скалярного произведения помогает определить вид угла:
\[
\begin{aligned}
\vct a\cdot\vct b&>0
&&\Longrightarrow&&
0^\circ\le\varphi<90^\circ,\\
\vct a\cdot\vct b&=0
&&\Longrightarrow&&
\varphi=90^\circ,\\
\vct a\cdot\vct b&<0
&&\Longrightarrow&&
90^\circ<\varphi\le180^\circ.
\end{aligned}
\]
...
Если после вычислений получено число вне этого диапазона, следует проверить арифметику и найденные длины векторов.
...
Поскольку \(B\), \(O\), \(D\) лежат на одной прямой в порядке \(B\! - \!O\! - \!D\) и \(BO=OD\), векторы \(\vect{BO}\) и \(\vect{OD}\) сонаправлены и равны:
\[
\vect{BO}=\vect{OD}.
\]
...
\begin{enumerate}
    ...
    \item \textbf{Формула для косинуса угла используется при нулевом векторе.}
