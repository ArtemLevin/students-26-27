(() => {
  ...
  const data = window.KIRIL_PHYSICS_COMPETENCY_DATA;
  ...
  function renderFocus() {
    ...
    const upcoming = all.find(item => getLevel(item.id) === 0 && !isRepeat(item.id));
    const target = repeat || low || upcoming || all[0];

    els.focusTitle.textContent = target.title;
    els.focusGroup.textContent = `${target.groupCode} · ${target.groupName}`;

    if (isRepeat(target.id)) {
      els.focusText.textContent = 'Повторите перевод единиц и выполните короткую проверочную задачу без подсказки.';
